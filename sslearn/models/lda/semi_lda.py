...
class SemiSupervisedLinearDiscriminantAnalysis:
    def __init__(self, max_iter=1000, lr=1.0e-3, tol=1.0e-6):
    ...
    def fit(self, X, y, unlabeled_X):
        ...
        self.classes, class_indexes, class_cnt = np.unique(
        ...
        gradient_sup_part = np.asarray(
            [
                ...
                * multivariate_normal.pdf(
                    unlabeled_X, mean=sup_mu_k, cov=sup_covariance
                )
        ...
        for step in range(self.max_iter):
            ...
            unlabeled_weighted_class_sum = np.asarray(
                [(unlabeled_X * q_k.reshape(M, 1)).sum(axis=0) for q_k in soft_labels.T]
            )  # [K, f]

            semi_means = (
                sup_means * class_cnt.reshape((K, 1)) + unlabeled_weighted_class_sum
            ) / (class_cnt + soft_labels.sum(axis=0)).reshape(
                (K, 1)
            )  # [K, f]
            ...
            unlabeled_cov = np.zeros(
                shape=(unlabeled_X.shape[1], unlabeled_X.shape[1])
            )  # [f, f]
            for q_k, mu_k in zip(soft_labels.T, semi_means):  # [1, M], [1, f]
                weighted_centered_unlabeled_X = (unlabeled_X - mu_k) * np.sqrt(
                    q_k
                ).reshape(M, 1)
                unlabeled_cov += np.dot(
                    weighted_centered_unlabeled_X.T, weighted_centered_unlabeled_X
                )
            ...
            gradient_semi_part = np.asarray(
                [
                    prior_k
                    * multivariate_normal.pdf(unlabeled_X, mean=semi_mu_k, cov=semi_cov)
                    for prior_k, semi_mu_k in zip(semi_priors, semi_means)
                ]
            ).T  # [M, f]

            gradient = np.log(
                (gradient_semi_part + 1e-20) / (gradient_sup_part + 1e-20)
            )  # [M, f]
            ...
            if (
                prev_obj_function_value is not None
                and np.abs(prev_obj_function_value - obj_function_value) < self.tol
            ):
                break
        ...
        self._intercept = -0.5 * np.diag(np.dot(semi_means, self._coef)) + np.log(
            semi_priors
        )
        return self
